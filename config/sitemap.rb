...
SitemapGenerator::Sitemap.create do
  ...
  # Add all chapters
  Chapter.includes(:slugs).find_each do |chapter|
    add "/#{chapter.chapter_number}", priority: 1

    chapter.slugs.each do |slug|
      add "/#{slug.slug}", priority: 1
    end
  ...
  Verse.find_each do |verse|
    ...
    add "/#{verse.verse_key}",  priority: 0.8
  ...
  # Static pages
  %w(about_us apps donations help_and_feedback developers).each do |url|
    add "/pages/#{url}", priority: 0.3
  end
