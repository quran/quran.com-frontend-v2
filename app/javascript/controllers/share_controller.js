...
export default class extends Controller {
  ...
  addButton(icon, name, options) {
    ...
    this.shareButtons.append(
      `<span class='share-icon ${icon} mr-6 mb-4'><a target='_blank' rel='noopener' title='Share on ${name}' class='fa fa-${icon}' ${linkOptions}></a><span class='text-muted fs-sm'>${name}</span></span>`
    );
  }
